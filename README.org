...
#+BEGIN_SRC shell
  sudo ./install.sh
#+END_SRC
- You may wish to install it locally instead though
...
MYLIBS = -I$(DIR)
PLPLOT = -lplplotfortran -lplplot $$(pkg-config --cflags --libs plplot-fortran) # Comment out if PLplot wasn't installed
