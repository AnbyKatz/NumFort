...
#+BEGIN_SRC shell
  ...
  sudo ./install.sh
#+END_SRC
- You may wish to install it locally instead though (hit option 3 i think after typing ./install.sh)
...
#+END_SRC
follow the prompts and it should install. Say yes to append the path to your .bashrc. Add the following lines in your .bashrc as well
#+BEGIN_SRC shell
  alias py="~/anaconda3/bin/python3.6"
#+END_SRC
now type "py" and the python command line should pop up. Should say anaconda as well, type C-d to exit out of the shell.
*** Matplotlib & Numpy
This should come with matplotlib, numpy and math. To test this open a python shell with py and type the following
#+BEGIN_SRC python
...
#+END_SRC
Let me know if anaconda doesn't auto install matplotlib and numpy
** NumFort
...
- The bash file /recompileEX.sh/ in the repository can be used to recompile these libraries if you should choose to edit and add. Simply run with ./recompileEX.sh whilst in the directory (Make sure to edit the paths to the correct ones
...
#+END_SRC
** pythonPlot
python plotting wrappers, simply copy /pyplots.py/ (or customPlot.py if you're ready for danger) into the necessary directory and use the following command.
