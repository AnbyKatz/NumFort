...
* Description
numFort is a numerical library for FORTRAN that includes quadpack, lapack and PLplot source code. It enables the use of handy mathematical macros such as linspace.
...
Call PLplot by using the subroutine plot(), It can be then called under a normal syntax like plot(x,y,xlabel,ylabel,title) or even plot(MATRIX). Make sure that you either include all axes labels and title or neither.
